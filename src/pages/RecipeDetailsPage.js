...
const RecipeDetailsPage = ({ recipeId, onEdit, onBack }) => {
  ...
  const getIngredientName = (ing) => ing.name || ing.ingredient?.name || "";

  const formatIngredientLine = (ing) => {
    const qty = ing.quantity;

    // αν δεν υπάρχει ποσότητα, δείξε μόνο όνομα
    if (qty == null) return getIngredientName(ing);

    const unitKey = toUnitKey(ing.measurementUnit);
    return `${qty} ${formatUnit(qty, unitKey)} ${getIngredientName(ing)}`;
  };
  ...
  if (!recipe)
    return <div className={classes.container}>Η συνταγή δεν βρέθηκε.</div>;
    ...
    <div className={classes.container}>
      ...
      <div className={classes.headerSection}>
        ...
        <div className={classes.topActionBar}>
          <button
            className={`${classes.actionBtn} ${classes.btnExecute}`}
      ...
      <div className={classes.metaBar}>
        <div className={classes.metaItem}>
          <div className={classes.metaIconWrapper}>
            <Clock size={20} />
          </div>
          <span>
            {recipe.totalDuration}{" "}
            {recipe.totalDuration === 1 ? "λεπτό" : "λεπτά"}
          </span>
        </div>
              ...
              recipe.recipeIngredients.map((ing, index) => (
                <li key={index} className={classes.ingredientItem}>
                  ...
                  <span>
                    <strong>
                      {ing.quantity}{" "}
                      {formatUnit(ing.quantity, ing.measurementUnit)}
                    </strong>{" "}
                ...
                .map((step, index) => (
                  <div key={index} className={classes.stepItem}>
                    <div className={classes.stepHeaderLine}>
                      ...
                      <span className={classes.stepDurationBadge}>
                        {step.duration}'
                      </span>
                    </div>
                      ...
                      {step.stepIngredients &&
                        step.stepIngredients.length > 0 && (
                          <div className={classes.stepIngredientsList}>
                            <span className={classes.stepIngLabel}>
                              Υλικά Βήματος:
                            </span>
      ...
      <div className={classes.sectionCard} style={{ marginTop: "30px" }}>
        ...
        <PhotoGallery recipeId={recipeId} allowDelete={false} />
      </div>
    </div>
  );
};
